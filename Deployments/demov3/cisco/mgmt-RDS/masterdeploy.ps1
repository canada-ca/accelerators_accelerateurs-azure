...
Param(
    ...
    [string]$containerName = "demo-mgmt-rds",
...
New-AzureRmDeployment -Location $Location -Name $containerName -TemplateUri "https://raw.githubusercontent.com/canada-ca/accelerators_accelerateurs-azure/master/Templates/arm/masterdeploy/20190319.1/masterdeployrg.json" -TemplateParameterFile (Resolve-Path -Path "$PSScriptRoot\parameters\masterdeployrg.parameters.json") -baseParametersURL "https://$deploymentStorageAccountName.blob.core.windows.net/$containerName/" -parametersSasToken $token -Verbose;

Write-Host "There was no deployment errors detected. All look good."
Write-Host ""
Write-Host "Next steps:"
Write-Host ""
Write-Host "Add DNS entry pointing to public ip of firewall for the RDS gateway. For example: gateway.mgmt.demo.ca"
...
Write-Host "https://gateway.mgmt.demo.ca/rdweb"