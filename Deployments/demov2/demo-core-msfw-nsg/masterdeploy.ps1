...
New-AzureRmDeployment -Location $Location -Name Base-Infrastructure-$containerName -TemplateUri "https://azpwsdeployment.blob.core.windows.net/library/arm/masterdeploy/20190319.1/masterdeploysub.json" -TemplateParameterFile (Resolve-Path -Path "$PSScriptRoot\parameters\masterdeploysub.parameters.json") -baseParametersURL "https://$deploymentStorageAccountName.blob.core.windows.net/$containerName/" -parametersSasToken $token -Verbose;
...
Write-Host
Write-Host "Connect to the demo website using a web browser at http://$corePUBLICip"
Write-Host "Connect to the docker server using SSH to $corePUBLICip"